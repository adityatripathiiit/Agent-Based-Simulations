...
to go
  ...
  ask patches with [pcolor = red] [
    ...
    ask neighbors with [pcolor = green] [
      ...
      if (direction = 45 ) [
        set probability probability - ( (south-wind-speed + west-wind-speed) / sqrt(2) )
      ]
      ...
      if random 100 < probability [
        ...
        if big-jumps? [
          let target patch-at ((random west-wind-speed) / 5) ((random south-wind-speed) / 5)
...
@#$#@#$#@
## What changed concerning the ‘tipping point’ between ‘live forest’ and ‘burned forest’ after the modification in 2?
Note: Since we just need to see the effect of tipping point on the change of Q2, we calcuate the tipping point of density by keeping the wind speeds at zero for all the directions. 
Also, the tipping point is for density, therefore, we have set the probability of spread to be constant (100% in my case) 
* The tipping point in original case came out to be around 58-61.
* The tipping point after modification case came out to be around 38-41. 

Note that the tipping point is a range since the position of forests patches are begin assigned randomly. 

The above results makes sense, as the tipping point reduced significantly after the modification of Q2. It is because now each agent has 8 neighbours unlike in the original model where there were 4 neighbours. Since the neighbours have increase, it is easier for the fire to spread across the forest. And hence we see that we need to decrease the density be approximate 20 % to find the tipping point for live vs burnt forest. 
@#$#@#$#@
default
true
0
