globals
[
  infected            ;; number of infected people
  infected-yesterday  ;; number of infected the previous day
  cured               ;; number of cured people
  dead .              ;; number of dead
]

turtles-own
[
  incubation-period   ;; number of days for the virus to become infectious in a person
  infection-length    ;; How long the person has been infected
  infectioness-dur    ;; number of days the virus is infectious in a person
  recovery-time       ;; time to recovery after infection
  onset-to-dead       ;; time do daed after infection
]

to setup
  clear-all
  ;; set infected [2 4 6 9 13 21 30 39 41 59 78 112 169 245 331 448 642 785 1020 1280 1600 2060 2362 2995 3544 4268]

  create-turtles density * (1 + max-pxcor - min-pxcor) * (1 + max-pycor - min-pycor)     ;; create a density of agents in the world
  [
    setxy random-xcor random-ycor
    set shape "person"
    set color white

    set incubation-period random-gamma 30.244 5.930

    set onset-to-dead random-gamma 785.422 41.777

    set recovery-time random-gamma 1743.11 70.57

    set infectioness-dur random-gamma 68.145 10.483
  ]
  ...
  ask one-of turtles
    [
      set color red
      set infection-length 5
    ]

  set infected-yesterday 0
  set cured 0
  set dead 0
...
to go
  ...
  ;; make the infected infect, recover or die
  ...
  set infected-yesterday infected
...
to infect
  if infection-length - incubation-period >= 0 and infection-length - incubation-period < infectioness-dur [
   let nearby-uninfected (turtles-on neighbors) with [ color = white ]
     let s count nearby-uninfected
     ask n-of (infectioness * s) nearby-uninfected [ set color red ]
  ]
end
...
to maybe-recover-or-die
  ...
  ;; if over validity period see if will die
  if (infection-length >  (onset-to-dead)) and (random-float 100 < mortality)
    [ set dead dead + 1
      die
    ]

  ;; person where ment to recover
...
"Cured" 1.0 0 -13840069 true "" "plot cured"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if infected-yesterday > 0 [plot infected / infected-yesterday]"
"pen-1" 1.0 0 -2674135 false "" ";; plot a threshold line -- an attendance level above this line makes the bar\n;; is unappealing, but below this line is appealing\nplot-pen-reset\nplotxy 0 1\nplotxy plot-x-max 1"
...
<experiments>
  <experiment name="experiment_1" repetitions="30" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="infectioness" first="0.1" step="0.1" last="1"/>
  ...
  <experiment name="experiment_3" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>infected</metric>
    <metric>cured</metric>
    ...
    <enumeratedValueSet variable="infectioness">
      <value value="0.9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="mortality">
      <value value="1.4"/>
  ...
  <experiment name="experiment_2" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="speed" first="1" step="0.5" last="4"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
